...
const checkout = async (req, res) => {
  try {
    // denata thiyena cart eka hoyanawa
    ...
    await order.save();//order database eke save karanawa
...
=======
    ...
    // Order eka heduwata passe cart eka his karanawa
>>>>>>> Stashed changes
    cart.items = [];
    await cart.save();
    ...
    res.status(201).json({ msg: 'Order placed successfully!', order });  //order eka frontend ekata ywnawa
...
const getAllOrders = async (req, res) => {
  try {
    const orders = await Order.find()
      .populate('userId', 'firstName lastName email')//orders walata adala okkoma user details gnnawa
