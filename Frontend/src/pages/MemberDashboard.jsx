// src/pages/MemberDashboard.jsx (සම්පූර්ණ)
...
// ★ NEW: My submitted events (inline list) – additive only
...
const MemberDashboard = () => {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      // Update context & local storage with latest data
      login(data);
    } catch (err) {
  ...
  return (
    <div className="container mx-auto max-w-5xl p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Right: Sport Registrations List */}
              ...
              {profileData?.playerProfiles && profileData.playerProfiles.length > 0 ? (
                <ul className="space-y-3">
                  {profileData.playerProfiles.map(p => (
                    <li key={p._id} className="text-gray-800 bg-gray-100 p-3 rounded-md">
                      {p.sportName} <span className="text-gray-500 text-sm">({p.skillLevel})</span>
                    </li>
                  ))}
                </ul>
              ) : (
