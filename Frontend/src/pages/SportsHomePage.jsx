...
// ★★★ 1. එක් එක් ක්‍රීඩාවට අදාළ, උසස් තත්ත්වයේ, Creative පින්තූර සමඟ යාවත්කාලීන කර ඇත ★★★
const sportsData = [
    { 
        name: 'Tennis', 
        ...
        icon: <i className="fa-solid fa-table-tennis-paddle-ball text-4xl text-white"></i> // උදාහරණයක් ලෙස icon එකක්
    },
    { 
        name: 'Cricket', 
            ...
            <div className="mx-auto max-w-7xl px-4 py-12 md:py-16">
                <div className="mx-auto max-w-7xl px-4">
                  <div className="rounded-2xl p-6 bg-[#0D1B2A] text-white border border-white/10 mb-8 text-center">
                    ...
                    <p className="text-white/80 mt-2 max-w-2xl mx-auto">Find the perfect sport that fits your passion and energy.</p>
                  </div>
                </div>
                
                {/* ★★★ 2. The Club පිටුවේ පෙනුමට ගැලපෙන, ආකර්ෂණීය Card ව්‍යුහය ★★★ */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {sportsData.map(sport => (
                        <div key={sport.name} className="group relative rounded-2xl overflow-hidden text-white flex flex-col h-72 border border-slate-200/80 bg-slate-900 shadow-sm hover:shadow-lg hover:ring-2 hover:ring-emerald-500/40 transition">
                            ...
                            <div className="relative z-10 flex flex-col h-full p-5 md:p-6">
                                {sport.icon} {/* අලුතින් එකතු කළ icon එක මෙහි පෙන්වයි */}
                                ...
                                <div className="flex items-center justify-center space-x-4">
                                    <Link to={sport.detailsLink} className="w-full text-center text-white/90 border border-white/30 bg-white/0 hover:bg-white/10 font-semibold py-2 px-4 rounded-lg transition-colors whitespace-nowrap">View Details</Link>
                                    <button onClick={() => handleRegisterClick(sport.name)} className="w-full text-center bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors whitespace-nowrap">
                                        Register
                                    </button>
                                </div>
                            </div>
